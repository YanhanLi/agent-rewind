...
```bash
npx --yes github:YanhanLi/agent-rewind doctor /绝对路径/允许访问的目录
npx --yes github:YanhanLi/agent-rewind config claude /绝对路径/允许访问的目录
```

第二条命令会生成 Claude Desktop 配置：
...
Run `npx --yes github:YanhanLi/agent-rewind doctor /absolute/allowed/directory` before configuring Claude Desktop. See the Chinese section above for the full configuration and safety boundaries.
