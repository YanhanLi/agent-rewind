...
历史按 change set 建立本地索引。审批页轮询时由 SQLite 汇总状态和精确总数，Node.js 每组只解析前 5 个动作与路径；完整记录仅在点击 `Load all details` 后读取。升级已有数据目录时，旧记录会在一个 SQLite 写事务内自动回填索引；即使单个 change set 超过 500 个动作，历史计数、检查和撤销仍会覆盖全部动作，且大型任务组不会被每秒重复解析、传输和重绘。所有带本地历史或恢复信息的 GET 响应均标记为 `Cache-Control: no-store`。
...
- SQLite-aggregated five-item history previews with exact counts and on-demand complete details;
