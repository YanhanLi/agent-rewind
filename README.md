...
Agent Rewind 是一个本地 MCP 文件操作代理。它会在 Agent 写入、编辑、移动或删除文件以及创建目录之前展示变更，让用户明确批准，并为已经执行的操作保存可验证的撤销记录。
