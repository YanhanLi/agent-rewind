...
const previewCache=new Map();const readinessCache=new Map();let actionInFlight=false;
...
const canUndo=x=>x.status==='applied'||x.status==='partial';const stateKey=x=>x.status+'|'+x.updatedAt;const readiness=x=>{const cached=readinessCache.get(x.id);if(!cached||cached.key!==stateKey(x))return '<div class="readiness">Undo readiness has not been checked. Checking reads current paths and verifies required local snapshots.</div>';const r=cached.result;return \`<div class="readiness \${esc(r.status)}">\${esc(r.message)}\${r.target?' '+esc(r.target):''} Checked \${new Date(r.checkedAt).toLocaleTimeString()}; execution will verify again.</div>\`};
const undoControls=x=>canUndo(x)?\`<button onclick="checkUndo('\${x.id}','\${stateKey(x)}',this)">\${readinessCache.has(x.id)?'Check again':'Check undo'}</button><button class="undo" onclick="post('/api/change-sets/\${x.id}/undo',this)">Undo set</button>\`:'';
...
const recoveredCard=x=>{const previews=previewCache.get(x.id);return \`<div class="item recovered"><div class="row"><div><div class="summary">\${setTitle(x)}</div><div class="meta">\${setMeta(x)}</div></div><div><span class="status recovery-status">needs review</span>\${undoControls(x)}<button class="keep" onclick="post('/api/change-sets/\${x.id}/review',this)">Keep changes</button></div></div><div class="notice">Agent Rewind found this change after an interrupted operation. Review the snapshot-backed evidence before keeping or undoing it.</div>\${canUndo(x)?readiness(x):''}\${previews?previews.map(p=>\`<div class="preview"><div class="meta">\${esc(p.path)} · \${esc(p.kind)}</div><pre>\${esc(p.detail)}</pre></div>\`).join(''):'<div class="meta">Loading snapshot evidence...</div>'}\${actions(x)}</div>\`};
async function renderRecovered(items){document.querySelector('#recovered').innerHTML=items.length?items.map(recoveredCard).join(''):'<div class="empty">No interrupted changes need review.</div>';syncBusy();await Promise.all(items.filter(x=>!previewCache.has(x.id)).map(async x=>{try{const r=await fetch('/api/change-sets/'+x.id+'/recovery-preview',{headers});if(r.ok)previewCache.set(x.id,(await r.json()).previews)}catch{}}));document.querySelector('#recovered').innerHTML=items.length?items.map(recoveredCard).join(''):'<div class="empty">No interrupted changes need review.</div>';syncBusy()}
